...
class MFLogger{
  ...
  /// @nodoc
  static void enter({@required String className, @required String methodName, @required String lineNumber}){
    className = DART + "-" + className;
    methodName = methodName + "-"+ lineNumber;
    _channel.invokeMethod(LOGGER_ENTER, <String,dynamic>{CLASSNAME: className, METHODNAME: methodName});
  }

  /// @nodoc
  static void exit({@required String className, @required String methodName, @required String lineNumber}){
    className = DART+ "-" + className;
    methodName = methodName + "-"+ lineNumber;
  ...
  void trace({@required String message, Map metadata}) async {
    _channel.invokeMethod(LOGGER_TRACE,<String, dynamic>{PACKAGE_NAME: this.packageName, MESSAGE: message, METADATA: metadata});
  }

  /// Logs the [message] at [DEBUG] level with any optional [metadata]
  void debug({@required String message, Map metadata}) async {
    _channel.invokeMethod(LOGGER_DEBUG,<String, dynamic>{PACKAGE_NAME: this.packageName, MESSAGE: message, METADATA: metadata});
  }

  /// Logs the [message] at [VERBOSE] level with any optional [metadata]
  void log({@required String message, Map metadata}) async {
    _channel.invokeMethod(LOGGER_LOG,<String, dynamic>{PACKAGE_NAME: this.packageName, MESSAGE: message, METADATA: metadata});
  }

  /// Logs the [message] at [INFO] level with any optional [metadata]
  void info({@required String message, Map metadata}) async {
    _channel.invokeMethod(LOGGER_INFO,<String, dynamic>{PACKAGE_NAME: this.packageName, MESSAGE: message, METADATA: metadata});
  }
  ...
  void error({@required String message, Map metadata}) async {
    _channel.invokeMethod(LOGGER_ERROR,<String, dynamic>{PACKAGE_NAME: this.packageName, MESSAGE: message, METADATA: metadata});
  }
